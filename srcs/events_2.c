...
/*   Updated: 2025/06/30 20:02:16 by eschmitz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	reset_view(t_fdf *fdf)
{
	...
	fdf->offset_x = 0;
	fdf->offset_y = 0;
	ft_printf("🏠 Vue remise a zero (zoom: %.2f)\n", fdf->zoom);
	render_map(fdf);
}
...
void	handle_utility_keys(int key, t_fdf *fdf)
{
	if (key == KEY_R)
	{
		ft_printf("🔄 Rafraichissement de l'affichage...\n");
		render_map(fdf);
	}
	else if (key == KEY_C)
	{
		...
		ft_printf("🎨 Couleur de fond changee (test rafraichissement)\n");
		render_map(fdf);
	}
...
int	handle_key(int key, t_fdf *fdf)
{
	...
	{
		ft_printf("👋 Fermeture du programme...\n");
		handle_close(fdf);
	}
